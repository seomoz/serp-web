...
const server = http.createServer(async (req, res) => {
  ...
  try {
    switch (true) {
      case /^\/public\/css/.test(url): {
        res.setHeader('Content-Type', 'text/css');
        return fs.createReadStream(url.replace('\/public\/css\/', path.join(fsroot, 'build/css/'))).
          pipe(res).
          on('error', (err) => {
            res.statusCode = 404;
      ...
      case /^\/api\/search/.test(url): {
        try {
          ...
          const result = await feedstore.fetchResultsByTopic({ keyword, type });
          res.setHeader('Content-Type', 'application/json');
