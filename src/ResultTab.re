...
let make = (~entries) => {
  ...
  let drawerControl = (topic, status) => {
    switch status {
      | Some(Open) => <a onClick={toggleDrawer(topic)}>{React.string("[-]")}</a>
      | Some(Closed) => <a onClick={toggleDrawer(topic)}>{React.string("[+]")}</a>
  ...
  let renderEntry = entry => {
    ...
    <li>
      <span>
        {drawerControl(topic, Map.get(drawers, topic))} {React.string(" " ++ topic)}
      </span>
      {topics}
    </li>
  };
